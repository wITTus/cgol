...
impl Game {
    ...
    pub fn insert(&mut self, pattern: Field<bool>) {
...
impl From<Field<bool>> for Game {
    fn from(pattern: Field<bool>) -> Self {
        let mut field = Game::with_size(pattern.rows, pattern.columns);
